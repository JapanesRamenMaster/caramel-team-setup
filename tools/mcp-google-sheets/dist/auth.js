...
export async function authenticateGoogle() {
    ...
    // Domain-wide delegation: impersonate a user via GOOGLE_SUBJECT
    const googleSubject = process.env.GOOGLE_SUBJECT;
    ...
    if (serviceAccountPath && existsSync(serviceAccountPath)) {
        try {
            ...
            if (googleSubject) {
                authOptions.clientOptions = { subject: googleSubject };
            }
    ...
    try {
        ...
        const adcOptions = { scopes: SCOPES };
        if (googleSubject) {
            adcOptions.clientOptions = { subject: googleSubject };
            console.error(`Using domain-wide delegation, impersonating: ${googleSubject}`);
        }
        const auth = new google.auth.GoogleAuth(adcOptions);
