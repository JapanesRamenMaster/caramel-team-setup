...
SELECT DATE_SUB(DATE(first_kst), INTERVAL WEEKDAY(first_kst) DAY) AS time, COUNT(*) AS target_first_reservation_users
FROM first_request
WHERE first_kst >= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 6 WEEK)
