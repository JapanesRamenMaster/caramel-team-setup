...
first_only AS (
  SELECT fp.user_id, fp.ptype, DATE_SUB(DATE(DATE_ADD(fp.ts, INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE_ADD(fp.ts, INTERVAL 9 HOUR)) DAY) AS `time`
  FROM first_purchase_all fp JOIN target_users tu ON tu.user_id = fp.user_id
  WHERE fp.ptype IN ('VOUCHER','SUBSCRIPTION','PACKAGE')
)
SELECT `time`,
  COUNT(DISTINCT CASE WHEN ptype='VOUCHER' THEN user_id END) AS voucher_cnt,
  COUNT(DISTINCT CASE WHEN ptype='PACKAGE' THEN user_id END) AS package_cnt,
  COUNT(DISTINCT CASE WHEN ptype='SUBSCRIPTION' THEN user_id END) AS subscription_cnt
FROM first_only
WHERE `time` >= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 6 WEEK)
  AND `time` < DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY)
