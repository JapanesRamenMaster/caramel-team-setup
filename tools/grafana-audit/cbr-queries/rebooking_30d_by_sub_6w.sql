-- 30일 재예약율: 구독자 vs 비구독자 (6주 rolling, 주차별)
-- 두 개의 시리즈를 반환: subscriber, non_subscriber
WITH lu AS (
...
washes AS (
  SELECT r.id AS rid, r.user_id, DATE_ADD(r.washed_at, INTERVAL 9 HOUR) AS w,
    DATE_SUB(DATE(DATE_ADD(r.washed_at, INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(r.washed_at, INTERVAL 9 HOUR))) DAY) AS wk
...
classified AS (
  SELECT w.rid, w.user_id, w.w, w.wk,
...
SELECT wk AS time, seg AS segment,
  ROUND(100.0 * SUM(CASE WHEN has_next THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 1) AS rebook_30d_pct
FROM classified
WHERE wk >= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 10 WEEK)
  AND wk <= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 5 WEEK)
GROUP BY wk, seg
