...
classified AS (
  SELECT wu.wk,
    CASE WHEN EXISTS(SELECT 1 FROM subscription s WHERE s.user_id=wu.user_id AND s.status='ACTIVE' AND s.started_at <= wu.last_w)
         THEN 'subscriber' ELSE 'non_subscriber' END AS seg
  FROM weekly_users wu
)
SELECT wk AS time,
  COUNT(DISTINCT CASE WHEN seg='subscriber' THEN 1 END) AS dummy,
...
FROM classified
WHERE wk >= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 6 WEEK)
GROUP BY wk
ORDER BY time
