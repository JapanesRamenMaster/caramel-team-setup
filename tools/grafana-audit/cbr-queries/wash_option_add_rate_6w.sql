...
SELECT
    STR_TO_DATE(
        DATE_FORMAT(
            DATE_SUB(
                DATE_ADD(wr.reservation_datetime, INTERVAL 9 HOUR),
                INTERVAL WEEKDAY(DATE_ADD(wr.reservation_datetime, INTERVAL 9 HOUR)) DAY
            ), '%Y-%m-%d'
        ), '%Y-%m-%d'
    ) AS time,
    ROUND(COUNT(DISTINCT rwo.reservation_id) / COUNT(DISTINCT wr.reservation_id) * 100, 1) AS '옵션 추가율'
