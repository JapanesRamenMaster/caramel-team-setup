-- 신규 vs 재구매 세차당 매출 (12개월 rolling, 주차별)
-- Grafana Time Series 패널용
-- 신규 = 해당 예약이 유저의 첫 번째 완료 세차
WITH
  live_users AS (
...
SELECT
    ...
    ROUND(SUM(CASE WHEN fw.first_reservation_id = rv.reservation_id THEN rv.sale_total ELSE 0 END)
          / NULLIF(COUNT(DISTINCT CASE WHEN fw.first_reservation_id = rv.reservation_id THEN rv.reservation_id END), 0)) AS '신규 세차당 매출',
    ROUND(SUM(CASE WHEN fw.first_reservation_id != rv.reservation_id THEN rv.sale_total ELSE 0 END)
          / NULLIF(COUNT(DISTINCT CASE WHEN fw.first_reservation_id != rv.reservation_id THEN rv.reservation_id END), 0)) AS '재구매 세차당 매출'
FROM reservation_revenue rv
JOIN reservation r ON r.id = rv.reservation_id
JOIN first_wash fw ON fw.user_id = r.user_id
