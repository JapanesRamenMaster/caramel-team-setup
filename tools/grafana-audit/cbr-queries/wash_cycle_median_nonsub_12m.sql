...
WITH
  ...
  ranked AS (
    SELECT
      CAST(DATE_FORMAT(DATE_ADD(g.reservation_datetime, INTERVAL 9 HOUR), '%Y-%m-01') AS DATE) AS time,
      g.gap_days,
      ROW_NUMBER() OVER (
        PARTITION BY CAST(DATE_FORMAT(DATE_ADD(g.reservation_datetime, INTERVAL 9 HOUR), '%Y-%m-01') AS DATE)
        ORDER BY g.gap_days
      ) AS seq,
      COUNT(*) OVER (
        PARTITION BY CAST(DATE_FORMAT(DATE_ADD(g.reservation_datetime, INTERVAL 9 HOUR), '%Y-%m-01') AS DATE)
