-- 3개월 재구매율: 유료 1회 (6주 rolling, 주차별)
-- Grafana 패널용. time = 코호트 주차 (첫 세차 시점). 90일 관찰 확정 코호트만 표시.
WITH live_users AS (
...
cohort_stats AS (
  ...
  WHERE fw.acq_segment = 'paid' AND fw.purchase_type = 'onetime'
  GROUP BY fw.cohort_week
  HAVING DATE_ADD(cohort_week, INTERVAL 90 DAY) <= DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))
)
SELECT
  cohort_week AS time,
  ROUND(100.0 * repurchased / NULLIF(cohort_size, 0), 1) AS repurchase_rate
FROM cohort_stats
WHERE cohort_week >= DATE_SUB(
  DATE_SUB(
