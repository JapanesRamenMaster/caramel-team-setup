...
first_only AS (SELECT user_id, type, CAST(DATE_FORMAT(paid_kst, '%Y-%m-01') AS DATE) AS `time` FROM first_payments WHERE rn = 1 AND paid_kst >= DATE_FORMAT(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL 11 MONTH), '%Y-%m-01'))
SELECT `time`, COUNT(DISTINCT CASE WHEN type='VOUCHER' THEN user_id END) AS voucher_cnt, COUNT(DISTINCT CASE WHEN type='PACKAGE' THEN user_id END) AS package_cnt, COUNT(DISTINCT CASE WHEN type='SUBSCRIPTION' THEN user_id END) AS subscription_cnt
FROM first_only GROUP BY `time` ORDER BY `time`;
