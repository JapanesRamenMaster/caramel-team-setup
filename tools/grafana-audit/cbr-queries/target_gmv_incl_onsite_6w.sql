...
SELECT DATE_SUB(DATE(ts), INTERVAL WEEKDAY(ts) DAY) AS time, ROUND(SUM(amt)) AS target_purchase_amount
FROM combined
WHERE ts >= DATE_SUB(DATE_SUB(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR)), INTERVAL WEEKDAY(DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 9 HOUR))) DAY), INTERVAL 6 WEEK)
