...
**v7의 핵심:** 마스터 30종이 `masters/*.html` 스니펫 + `masters.css` 클래스로 **이미 코드화**되어 있다. 매 덱마다 마스터 PDF를 렌더해 픽셀을 추정하는 일은 없다 — 스니펫을 복사해 콘텐츠를 **캐스팅**한다.
...
5. **카드는 3층으로 쓴다 — 라벨 → 앵커(80px) → 불렛(24px).** 카드마다 `.card-anchor` 1개 필수. "라벨 + 28px 줄글" 2층 금지. 앵커로 뽑을 수치가 없으면 그건 카드가 아니다 (원칙 29 · layouts.md).
6. **같은 줄에서 폰트 크기를 섞지 않는다.** 앵커 옆 수식어는 `.anchor-note`로 다음 줄에. 단위는 크기가 아니라 색으로 낮춘다. 인라인 `font-size` 금지. 표 합계행은 `tbody > tr.total`(tfoot은 폰트가 커도 행이 줄어든다) (원칙 32).
...
```
| # | 앵커 (수치/6자 이내) | 불렛 (한 줄에 한 항목, ≤3개) | 버린 것 |
|---|---|---|---|
| 5 | 80.5% | 방문 세차 구독·회차권 / 거점 스테이션 추가 / 변동비 9,397원 | 구독 상품 구조 상세 |
```
...
읽어야 할 파일 (이게 전부다):
- `layouts.md` — 마스터 30종 카탈로그 + 모디파이어 + 스니펫·클래스 매핑 표
...
1. **기계 게이트**: `validate-deck.sh` PASS까지. findings(슬라이드·요소·px)가 좌표를 특정해주므로 VLM 왕복 없이 바로 수정.
   ...
   | `mixed-size-inline` | 같은 줄 폰트 크기 비 ≥1.25 | 수식어를 `.anchor-note`로 내린다 |
...
- [ ] 카드마다 앵커 1개(80px) + 불렛 한 줄에 한 항목 — 라벨+줄글 2층 금지 (원칙 29)
